...
class Patient:
    ...
    def simulate(self, sim_length):
        ...
        while self.HealthStatus!=3 and k  < sim_length:
            trans_probs = TRANS[self.Therapy][self.HealthStatus]
            empirical_dist = rndClasses.Empirical(trans_probs)
            new_state_index = empirical_dist.sample(self._rng)
            if self.HealthStatus==1:
                self.STROKE+=1
            cost=TRANS_COST[self.Therapy][self.HealthStatus]
            utility=TRANS_UTILITY[self.Therapy][self.HealthStatus]
...
report_CBA()
print("When willingness-to-pay is about more than 10000, i will recommend use the therapy.")
